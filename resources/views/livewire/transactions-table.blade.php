...
new class extends Component {
    ...
    public function mount(bool $isReceipt = true): void
    {
    ...
    public function with(): array
    {
        ...
        return [
            'headers' => [
                ...
                ['index' => 'recurred', 'label' => 'Recorrente?'],
            ...
            'rows' => Transaction::query()
                ...
                ->where('is_receipt', $this->isReceipt)
                ->orderBy('date')
...
<div>
                ...
                <form wire:submit="save">
                    <div>
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <x-toggle label="É recorrente?" color="lime" wire:model="form.is_recurred"/>
                        </div>

                        <x-button text="Criar" color="green" type="submit"/>
                    </div>
                </form>
            </x-modal>
    ...
    <div>
        <x-table :$headers :$rows paginate filter>
            @interact('column_recurred', $row)
            @if ($row->is_recurred)
                <x-badge color="green">Sim</x-badge>
            @else
                <x-badge color="red">Não</x-badge>
            @endif
            @endinteract
